...
def caption_image_beam_search(encoder, decoder, image_path, word_map, beam_size=3):
    ...
    # img = resize(img, (256, 256)) #######was imresize - comment out this line since it always generates the same caption
...
def visualize_att(image_path, seq, alphas, rev_word_map, smooth=True):
    ...
    for t in range(len(words)):
        if t > 50:
            break
        plt.subplot(int(np.ceil(len(words) / 5.0)), 5, t + 1)  ### added int func
