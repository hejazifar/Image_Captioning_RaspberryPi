...
if __name__ == '__main__':
    ...
    encoder.eval()
    torch.save(encoder,'EncoderPruned.pth')
    ...
    # summary(encoder,(3,256,256))
    '''
    myquantizedencoder = Encoder()
    sd3 = myencoder.state_dict()
    sd4 = myquantizedencoder.state_dict()
    for key in sd3.keys():
        if key in sd4.keys():
            sd4[key] = sd3[key].to_sparse()

    myquantizedencoder.load_state_dict(sd4)
    '''
    '''
    with torch.no_grad():
        for name, param in myencoder.named_parameters():
            if 'conv' in name:
                sparsified = param.to_sparse()
                param.copy_(sparsified)
                # print(param)
    '''
    #Checkpoint2 = {'encoder': myencoder, 'decoder': decoder}
    #torch.save(Checkpoint2, 'mehdi.pth')
    ...
    with torch.no_grad():
        for name, param in encoder.named_parameters():
            param[abs(param) < eps] = 0
        for named, paramd in decoder.named_parameters():
            paramd[abs(paramd) < eps] = 0
    '''
    ########################################## pruning
    ...
    with torch.no_grad():
        for name, param in encoder.named_parameters():
            if 'conv' in name:
                ...
                rand_tensor = np.random.rand(*shape) < 0.9 # 80percent of the weights are preserved
                pruned_tensor = torch.tensor(rand_tensor, device=device) * param
                param.copy_(pruned_tensor)
                #print(param)
    '''
    ...
    for name, module in encoder.named_modules():
        ...
        if isinstance(module, torch.nn.modules.conv.Conv2d):
            #print(module)
            #prune.random_unstructured(module, name='weight', amount=0.20)
            #prune.remove(module, 'weight')
            if list(module.children()) == []:
                leaves.append(module)
        # prune 40% of connections in all linear layers
        #elif isinstance(module, torch.nn.Linear):
        #    prune.random_unstructured(module, name='weight', amount=0.40)
        #    prune.remove(module, 'weight')
    parameters_to_prune = ((module, 'weight') for module in leaves if (isinstance(module, torch.nn.modules.conv.Conv2d) or isinstance(module, torch.nn.modules.Linear)))
    prune.global_unstructured(parameters_to_prune, pruning_method=prune.L1Unstructured, amount=0.2)
    '''
    '''
    parents = [module for module in encoder.modules()]
    leaves = []
    while parents != []:
        m = parents.pop(0)
        if list(m.children()) == []:
    ...
    parameters_to_prune = [(module, 'weight') for module in leaves if (isinstance(module, torch.nn.modules.conv.Conv2d) or isinstance(module,
